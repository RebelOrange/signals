...
* `examples`: Any example that shows how to use this OOT module goes here.
  Examples that are listed in CMake get installed into `share/rfnoc-ada_filt/examples`
  within the installation prefix.

* `rfnoc/fpga/ada_filt`: This directory contains the gateware for the HDL modules
  ...
  module name ('ada_filt') is included in the path. When installing this OOT
  module, these files get copied to the installation path's RFNoC package dir
  (e.g., `/usr/share/uhd/rfnoc/fpga/ada_filt`) so the image builder can find
  all the source files when compiling bitfiles that use multiple OOT modules.

* `gr-rfnoc_ada_filt`: An out-of-tree module for GNU Radio, which depends on this
  RFNoC out-of-tree module and provides GNU Radio and GNU Radio Companion
  bindings for using the ada_filt block from within GNU Radio.
...
* `icores`: Stores full image core files. YAML files in this directory get
  ...
  an X310 image core with the ada_filt block included. You can build this image
...
* `include/rfnoc/ada_filt`: Here, all the header files for the block controllers
  ...
  e.g., these header files could get installed to `/usr/include/rfnoc/ada_filt`.
  By mirroring the path name, we can write
  `#include <rfnoc/ada_filt/ada_filt_block_control.hpp>` in our C++ source code, and
...
directory structure in this example, e.g., `/usr/share/uhd/rfnoc/fpga/ada_filt` will
have the same content as `./fpga/ada_filt`. This allows the image builder to work
...
To build and use the `rfnoc_ada_filt` Python module, `pybind11` must be installed.
...
```sh
cd /path/to/rfnoc-ada_filt/build
...
```sh
cd /path/to/rfnoc-ada_filt  # Switch to this file's directory
